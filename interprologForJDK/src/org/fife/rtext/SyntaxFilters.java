 ...
 * Includes some stuff from RTextUtilities
 */
...
public class SyntaxFilters implements SyntaxConstants {
	...
	public String getFilterString(String style) {
		...
		Iterator<String> i = getFiltersForStyle(style).iterator();
		while (i.hasNext()) {
			filterString.append(i.next()).append(' ');
		}
	...
	private static final String getSyntaxStyleForFileImpl(String fileName,
		...
		for (Iterator<Map.Entry<String,List<String>>> i=filters.entrySet().iterator(); i.hasNext(); ) {
	...
	// These were grabbed from org.fife.rtext.RTextUtilities:
