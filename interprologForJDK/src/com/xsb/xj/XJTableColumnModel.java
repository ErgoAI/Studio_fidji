...
/**
 * XJTableColumnModel extends DefaultTableColumnModel and adds the ability to
 * hide columns and add them back in.
 *
 *@author    Harpreet Singh
 *@version   $Id: XJTableColumnModel.java,v 1.1 2004/03/22 14:25:10 hsingh Exp $
 */
@SuppressWarnings("serial")
public class XJTableColumnModel extends DefaultTableColumnModel {
    /**
     ...
     */
    public XJTableColumnModel(XJAbstractListModel model) {

        for(int i = 0; i < model.getColumnCount(); i++) {
            TableColumn newColumn  = new TableColumn(i);
            newColumn.setHeaderValue(model.getColumnName(i));
            allTableColumns.addElement(newColumn);

            TermModel hidden       = model.cellGTs[i].findProperty("hidden");

            if(hidden == null) {
                //System.out.println("showing column:" + i + ":" + hidden);
     ...
     * status did not change. The call is also ignored if this is the only
     * visible column.
     *
     *@param column   The new columnVisible value
     *@param visible  its new visibility status
     *@return         Returns false if the column is the last column and was not
     *      hidden
     */
    public boolean setColumnVisible(TableColumn column, boolean visible) {
        int noVisibleColumns  = tableColumns.size();
        boolean result        = true;
        if(!visible) {
            if(noVisibleColumns > 1) {// dont hide the last column
                super.removeColumn(column);
            } else {
                result = false;
                System.err.println("XJTableColumnModel: Trying to hide last column.");
            }
        } else {
    ...
    /**
     * Maps the index of the column in the table model at <code>modelColumnIndex</code>
     * to the TableColumn object. There may me multiple TableColumn objects
     * showing the same model column, though this is uncommon. This method will
     * always return the first visible or else the first invisible column with
     * the specified index.
     *
     *@param modelColumnIndex  index of column in table model
     *@return                  table column object or null if no such column in
     *      this column model
     */
     ...
     *@return         visibility of specified column (false if there is no such
     *      column at all. [It's not visible, right?])
     */
    ...
    /**
     * Removes <code>column</code> from this column model. Posts <code>columnRemoved</code>
     * event. Will do nothing if the column is not in this model.
     *
     *@param column  the column to be added
     *@see           #addColumn
     */
     ...
     * Posts <code>columnMoved</code> event. Will not move any columns if <code>oldIndex</code>
     ...
     *@return             the number of columns in the <code>tableColumns</code>
     *      array
     *@see                #getColumns
     */
     ...
     *      enumeration.
     *@return             an <code>Enumeration</code> of the columns in the
     *      model
     */
     ...
     * . Position is the the index in all visible columns if <code>onlyVisible</code>
     * is true or else the index in all columns.
     *
